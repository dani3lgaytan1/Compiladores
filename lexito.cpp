...
const char* FILE_NAME="./archivo2.txt";
        ...
        switch(state){ // Analizar el estado actual
            case 0: // En el estado de lectura de cadena en nombre de, no ha ingresado ning?n aut?mata
                ...
                else if( C=='=' ) //igual s
                {
    			 ...
    			 else if( operadorAritmetico(C) )//operadores aritmeticos restantes 
                {
                    state = 0;
                ...
                break;
            case 1:
                if( esunaLetra(C) || esunDigito(C) || C == '_' ) //despues de la primera letra puede venir un numero, guion o letra
                {
                    state=1;//estado 1 del id 
                    token += C;
                    C = getChar(buf);
                }
                else{ //Verificar si es una palabra reservada 
                    word_num[token]++; // Cuenta el numero total de cada caract
            ...
            case 2: // numero 
                ...
                {//si ya no viene un numero pero esta en el estado 2 entonces es un numero reconocido 
                    cout<<"numero: "<<token<<"";
                    printf("  linea:%d\n",line_num );
            ...
            case 7://numero con punto decimal 
                 if(esunDigito(C)){//es necesario que venga un digito 2.
                    ...
                    //cout<<"< num, "<<num<<" >";
                 	...
                 	state=10;
                 	token+= C;//a�adir al token el numero
					C = getChar(buf); 
				 }
                break;
            case 8:
                 if(esunDigito(C)){//si veiene un digito es 3e
                 	...
                 	token+= C;
					C = getChar(buf); 
				 } else if(C == '-'){//si viene - es 7-e
				 	state=9;
                 	token+= C;
					C = getChar(buf); 
				 }else if( C=='+'){//si viene + es 5+e 
				 	state=9;
                 	token+= C;
					C = getChar(buf); 
				 }
            ...
            case 10:
                if(esunDigito(C))
                { //si viene digito entonces se queda en el estado 
            	    state = 10;
            	    token+=C;
                    C=getChar(buf);
                }
                else if(C=='e')
                {//si viene e entonces se va al estado 8
               	    state = 8;
               	    token+=C;
               	    C=getChar(buf);
                }else{
            ...
            case 11:
                if(esunDigito(C))
                { //Si es un digito se queda en el mimso estado 
                	state =11;
               	    token+=C;
               	    C=getChar(buf);
            ...
            case 16://caso ==
            	if(C=='='){
            		...
            		state = 0;
                    C=getChar(buf);
				}else{//si no es el caso entonces es solo = 
                     cout<<"< relop, = >"<<endl; 
            		state = 0;
                }
            break;	
            case 17://si es comillas ya acabo la cadena 
               if(C=='"'){
                 token+=C;
                    cout<<"<cadena, "<<token<<" >";
                    printf("%d\n",line_num );
